...
export default function PdfDocumentReader({ fileUrl, title, fitMode = 'page', scrollMode = 'paged' }) {
  ...
  const continuous = scrollMode === 'continuous'
  ...
  const viewportPadding = continuous ? 56 : fitMode === 'width' ? 24 : 32
  const pageWidth = Math.max(220, fitMode === 'width'
    ? viewportSize.width - viewportPadding
    : Math.min(
      viewportSize.width - viewportPadding,
      (viewportSize.height - viewportPadding) * pageAspect,
    ))

  return (
    <section className={`pdf-document-reader${continuous ? ' pdf-document-reader--continuous' : ''}`} aria-label={`${title} PDF 阅读器`}>
      <TransformWrapper initialScale={1} minScale={1} maxScale={4} centerOnInit wheel={{ disabled: true }} doubleClick={{ mode: 'reset' }}>
        {({ zoomIn, zoomOut, resetTransform }) => {
          ...
          const handleReset = () => {
            resetTransform()
            if (continuous) viewportRef.current?.scrollTo?.({ top: 0, left: 0 })
          }
          return <>
            <div className="pdf-document-reader__toolbar">
              {continuous ? (
                <span className="pdf-document-reader__page-count pdf-document-reader__page-count--wide">共 {pageCount || '—'} 页，向下滚动浏览</span>
              ) : (
                <>
              ...
              <button type="button" className="pdf-document-reader__fit" onClick={handleReset}>{fitMode === 'width' ? '适应宽度' : '适应整页'}</button>
            </div>
            ...
            }} aria-label={continuous ? 'PDF 内容。可上下滚动浏览，触屏可双指缩放。' : 'PDF 内容。可用左右方向键翻页，触屏可双指缩放和单指拖动。'}>
                  ...
                  {continuous ? (
                    <div className="pdf-document-reader__pages">
                      {Array.from({ length: pageCount }, (_, index) => (
                        <Page key={index + 1} pageNumber={index + 1} width={pageWidth} renderTextLayer renderAnnotationLayer onLoadSuccess={index === 0 ? (page) => {
