...
export default function HardPressboardListPage({ scopeType }) {
  ...
  const save = async () => {
    ...
    try {
      const result = isDevice ? await api.saveHardPressboardList(scopeType, scopeId, items.map((item, index) => ({
        pressboardRef: item.pressboardRef,
        baselineValue: Boolean(item.baselineValue),
        compareEnabled: item.compareEnabled !== false,
        sortOrder: index,
      }))) : await api.saveHardPressboardSelection(scopeType, scopeId,
        items.filter((item) => item.compareEnabled).map((item) => item.pressboardRef))
      applyData(result)
      setMessage(isDevice ? '硬压板基准清单已保存' : '硬压板校验项目已保存')
    } catch (err) {
    ...
    <div className="users-page setting-list-page">
      <div className="users-page__header">
        <div>
          ...
          <h2 className="users-page__title">{data ? `${data.scopeName} — ${scopeType === 'IED_DEVICE' ? '装置硬压板基准清单' : '硬压板校验项目'}` : '硬压板基准清单'}</h2>
          <p className="users-page__desc">{isDevice ? '硬压板项目由屏柜召唤或 Excel 导入生成，召唤后确认并保存才会生效。' : '硬压板基准统一来自装置清单，仅勾选本逻辑需要校验的项目。'}</p>
        </div>
      ...
      {loading ? <p className="users-page__loading">加载中…</p> : (
        <div className="users-page__table-wrap">
          <table className="users-page__table setting-list-page__table">
            <thead><tr><th>序号</th><th>硬压板名称</th><th>硬压板编号</th><th>参与比对</th><th>基准状态</th>{isDevice && <th>排序</th>}</tr></thead>
            <tbody>{items.length === 0 ? <tr><td colSpan={isDevice ? 6 : 5} className="users-page__empty-cell">{isDevice ? '当前装置暂无硬压板项目，请召唤或导入 Excel。' : '装置尚未配置硬压板清单，请先在装置层维护。'}</td></tr> : items.map((item, index) => (
