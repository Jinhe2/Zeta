...
/** 基础组合及整组实验共享的断面解析器；成员授权由调用方传入的历史成员集合限定。 */
public class LogicGroupSnapshotParser {
  private final ObjectMapper objectMapper;
  private final LogicSnapshotService logicSnapshotService;

  public LogicGroupSnapshotParser(ObjectMapper objectMapper, LogicSnapshotService logicSnapshotService) {
    this.objectMapper = objectMapper;
    this.logicSnapshotService = logicSnapshotService;
  }

  public List<MemberSummaryResponse> listMembers(String json, Set<Long> allowed) {
    JsonNode root = parseAndValidateRoot(json);
    ...
    for (JsonNode member : root.path("logics")) {
      ...
      if (!seen.add(summary.getLogicDiagramId())) throw invalidFormat("组合断面包含重复的 logicDiagramId");
      if (!allowed.contains(summary.getLogicDiagramId())) {
        throw new ResponseStatusException(HttpStatus.NOT_FOUND, "组合断面包含非本组合成员");
      }
      if (!"failed".equals(summary.getStatus())) buildMemberSnapshotJson(root, member, summary);
      result.add(summary);
    }
    result.sort(Comparator.comparingInt(MemberSummaryResponse::getOrder));
    return result;
  }

  public MemberDetailResponse getMember(Long snapshotId, Long groupId, String json,
      Set<Long> allowed, Long logicDiagramId) {
    if (!allowed.contains(logicDiagramId)) {
    ...
    if (!"failed".equals(summary.getStatus())) {
      sections = logicSnapshotService.parseSections(buildMemberSnapshotJson(root, member, summary));
    }
    return new MemberDetailResponse(snapshotId, groupId, summary.getLogicDiagramId(), summary.getLogicId(),
        summary.getOrder(), summary.getStatus(), summary.getExperimentPassed(), summary.getTotalTransitions(),
        summary.getErrorCode(), summary.getErrorMessage(), sections);
  }

  private JsonNode parseAndValidateRoot(String snapshotJson) {
    final JsonNode root;
    try {
