...
@Service
public class SoftPressboardListExcelService {
  private static final long MAX_FILE_SIZE = 5L * 1024L * 1024L;
  private static final String SHEET_NAME = "软压板基准清单";
  private static final String[] HEADERS = {
      "序号", "软压板名称", "软压板引用", "是否比对", "基准状态"
  };
  private final SoftPressboardListService listService;

  public SoftPressboardListExcelService(SoftPressboardListService listService) {
  ...
  public byte[] exportWorkbook(SettingListScopeType scopeType, Long scopeId) {
    ...
    try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
      ...
      int[] widths = {10, 36, 72, 14, 16};
      for (int i = 0; i < widths.length; i++) sheet.setColumnWidth(i, widths[i] * 256);
      workbook.write(output);
      return output.toByteArray();
    } catch (IOException ex) {
      throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "生成软压板基准清单 Excel 失败", ex);
    }
  }
  ...
  public ListResponse importWorkbook(
    ...
    try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
      ...
      for (int rowIndex = 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
        ...
        if (ref.isEmpty()) throw badRequest("第 " + displayRow + " 行软压板引用不能为空");
        if (!seen.add(ref)) throw badRequest("第 " + displayRow + " 行软压板引用重复：" + ref);
