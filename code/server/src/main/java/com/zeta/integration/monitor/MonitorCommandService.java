...
import com.zeta.business.entities.monitor.MonitorTask;
...
/** 向 monitord 发送命令并处理响应的核心服务。 维护 req_id → CompletableFuture 映射，实现请求-响应关联。 */
@Service
public class MonitorCommandService {
  ...
  private final ConcurrentHashMap<String, Map<String, Object>> latestResponses =
      new ConcurrentHashMap<>();
  ...
  private final java.util.concurrent.atomic.AtomicLong totalReceived =
      new java.util.concurrent.atomic.AtomicLong(0);
  private volatile long lastSentAt = 0;
  private volatile long lastReceivedAt = 0;
  private final ConcurrentHashMap<String, java.util.concurrent.atomic.AtomicLong> sentByCommand =
      new ConcurrentHashMap<>();

  /** 最近消息日志（环形缓冲，保留最近 20 条） */
  private final java.util.concurrent.ConcurrentLinkedDeque<Map<String, Object>> recentMessages =
  ...
  /** 读取指定 IED 入向虚回路的断链和异常状态。 */
  public CompletableFuture<ScreenQueueMessage> sendVirtualCircuitStatusRequest(
  ...
  public CompletableFuture<ScreenQueueMessage> startLogicMonitor(
      String iedName, String logicId, Long userId, String username) {
    Map<String, Object> data = new LinkedHashMap<>();
  ...
  private void createLogicSnapshotFromTask(Long taskId, String userIdStr) {
    ...
    snapshot.setTotalTransitions(
        task.getTotalTransitions() != null ? task.getTotalTransitions() : 0);
    ...
    log.info(
        "Created logic_snapshot id={} from monitor_task={} userId={}",
        snapshot.getId(),
        taskId,
        userIdStr);
  }
  ...
  public void handleResponse(ScreenQueueMessage message) {
    ...
    log.info(
        "Received monitord response: command={} req_id={} success={}",
        command,
        reqId,
        message.getSuccess());
  ...
  private void addRecentMessage(
      String direction, String command, String reqId, Map<String, Object> data) {
  ...
  private void fireAndForget(String command, Map<String, Object> data) {
    ...
    sentByCommand
        .computeIfAbsent(command, k -> new java.util.concurrent.atomic.AtomicLong(0))
        .incrementAndGet();
        ...
        () -> {
          ...
          if (pending != null && !pending.isDone()) {
            pending.completeExceptionally(
                new TimeoutException("命令超时（" + DEFAULT_TIMEOUT_SECONDS + "s）"));
          }
        },
