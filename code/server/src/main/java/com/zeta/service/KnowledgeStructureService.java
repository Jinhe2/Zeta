...
public class KnowledgeStructureService {
    ...
    public List<CabinetSummaryResponse> listCabinets() {
        ...
        for (Cabinet cabinet : cabinetRepository.findByEnabledTrueOrderBySortOrderAscIdAsc()) {
            int deviceCount = deviceRepository
                    .findByCabinetIdAndEnabledTrueOrderBySortOrderAscIdAsc(cabinet.getId())
                    .size();
    ...
    public List<DeviceSummaryResponse> listDevicesByCabinet(Long cabinetId) {
        ...
        for (Device device : deviceRepository.findByCabinetIdAndEnabledTrueOrderBySortOrderAscIdAsc(cabinetId)) {
    ...
    public List<ProtectionLogicBriefResponse> listProtectionLogicsByDevice(Long deviceId) {
        ...
        return protectionLogicRepository
                .findByDeviceIdAndEnabledTrueOrderBySortOrderAscIdAsc(deviceId)
                .stream()
    ...
    public KnowledgeTreeResponse getKnowledgeTree() {
        ...
        for (Cabinet cabinet : cabinetRepository.findByEnabledTrueOrderBySortOrderAscIdAsc()) {
            ...
            for (Device device : deviceRepository.findByCabinetIdAndEnabledTrueOrderBySortOrderAscIdAsc(cabinet.getId())) {
                List<ProtectionLogicBriefResponse> logics = protectionLogicRepository
                        .findByDeviceIdAndEnabledTrueOrderBySortOrderAscIdAsc(device.getId())
                        .stream()
                        .map(this::toProtectionLogicBrief)
                        .collect(Collectors.toList());
    ...
    private DeviceSummaryResponse toDeviceSummary(Device device) {
        int logicCount = protectionLogicRepository
                .findByDeviceIdAndEnabledTrueOrderBySortOrderAscIdAsc(device.getId())
                .size();
        Cabinet cabinet = device.getCabinet();
